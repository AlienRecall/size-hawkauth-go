...
func gen(id string, secret string, method string, path string, domain string) string {
  ts := time.Now().Unix()
...
func main() {
  sizeID := "c012543e9e"
  sizeSecret := "7fbda8fefe33eec041a3cbca366fab32"
  method := "GET"
  path := "/stores/size/carts?channel=android-app-phone"
  domain := "prod.jdgroupmesh.cloud"
  header := gen(sizeID, sizeSecret, method, path, domain)
  fmt.Println(header)
}
